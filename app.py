...
if "financeiro" not in st.session_state:
  st.session_state.financeiro = carregar_dados(
      FINANCE_FILE,
      [
          {
              "mes": "Janeiro/2026",
...
if "avisos" not in st.session_state:
  st.session_state.avisos = carregar_dados(
      AVISOS_FILE,
      {
...
if "regulamento" not in st.session_state:
  st.session_state.regulamento = carregar_dados(
      REGULAMENTO_FILE,
      [
          {
              "topico": "📌 1. Prioridade de Mensalistas",
              "regrinha": (
                  "Mensalistas confirmando até as 17:00 de segunda-feira têm"
                  " prioridade nas 15 vagas."
              ),
          },
          {
              "topico": "⏳ 2. Fila de Espera de Avulsas",
              "regrinha": (
                  "Avulsas entram na fila de espera. Após as 17:00, se"
                  " sobrarem vagas, sobem automaticamente."
              ),
          },
          {
              "topico": "⏰ 3. Fechamento da Lista",
              "regrinha": (
                  "A lista fecha rigidamente às 18:00 de toda segunda-feira."
              ),
          },
          {
              "topico": "🤝 4. Boa Convivência",
              "regrinha": (
                  "Respeito mútuo em campo e fora dele é obrigatório para"
                  " todas as atletas."
              ),
          },
      ],
...
if st.session_state.pagina_atual == "login":
  ...
  tab_entrar, tab_cad_jogadora, tab_cad_admin, tab_dev = st.tabs(
      ["🔑 Entrar", "📝 Criar Conta", "👑 Criar Conta Admin", "⚙️ Desenvolvedor"]
  )
      ...
      if st.form_submit_button("ENTRAR"):
        admin_encontrado = next(
            (
                ...
                if adm.get("login") == l_user
                and adm.get("senha") == l_pass
            ),
            None,
        )
        if admin_encontrado:
          ...
          user_found = next(
              (
                  ...
                  if j.get("login") == l_user and j.get("senha") == l_pass
              ),
              None,
          )
          if user_found:
      ...
      if st.form_submit_button("CADASTRAR ATLETA"):
        if c_nome and c_user and c_pass:
          if any(
            ...
            st.success(
                "Cadastro realizado com sucesso! O Administrador recebeu sua"
                " solicitação."
            )
        else:
...
# -----------------------------------------------------------------------------
  ...
  if st.session_state.pagina_atual == "dashboard":
    ...
    for reg in st.session_state.regulamento:
      st.markdown(
          f"<div class='card-team'><h4 style='color:"
          f" #F43F5E;'>{reg['topico']}</h4><p>{reg['regrinha']}</p></div>",
          unsafe_allow_html=True,
      )
    ...
    lista_atual = sorted(
        ...
        key=lambda x: x.get("dt_confirmacao", x.get("hora", "")),
    )
    ...
    with col_l1:
      ...
      for i, p in enumerate(confirmadas, 1):
        st.markdown(
            f"<div class='card-team'><b>{i}.</b> {obter_nome_p(p)}"
            f" `[{obter_tipo_p(p)}]` — <i>{obter_hora_p(p)}</i></div>",
            unsafe_allow_html=True,
        )
      ...
      for i, p in enumerate(espera, 1):
        st.markdown(
            f"<div class='card-team'><b>{i}º:</b> {obter_nome_p(p)}"
            f" `[{obter_tipo_p(p)}]`</div>",
            unsafe_allow_html=True,
        )
    ...
    with col_l2:
      if st.session_state.perfil_logado in ["Admin", "Dev"]:
        ...
        with st.form("form_add_manual"):
          ...
          atleta_escolhida = st.selectbox(
              "Selecione a Atleta",
              atativas_nomes if atativas_nomes else ["Nenhuma cadastradas"],
          )
          if st.form_submit_button("Incluir do Elenco"):
            if atativas_nomes and not any(
                obter_nome_p(p) == atleta_escolhida
        ...
        with st.form("form_add_externa"):
          st.write(
              "<b>Adicionar Convidada / Avulsa (Sem Cadastro)</b>",
              unsafe_allow_html=True,
          )
          nome_externa = st.text_input("Nome da Convidada")
          tipo_externa = st.selectbox(
              "Tipo da Convidada", ["Avulso", "Mensalista"], key="tipo_ext"
          )
          if st.form_submit_button("Incluir Convidada"):
            if nome_externa.strip():
              if not any(
                  obter_nome_p(p) == nome_externa.strip()
                  for p in st.session_state.presencas
              ):
                st.session_state.presencas.append({
                    "nome": nome_externa.strip(),
                    "hora": hoje_dt.strftime("%H:%M"),
                    "tipo": tipo_externa,
                    "dt_confirmacao": hoje_dt.isoformat(),
                })
        ...
        for p in st.session_state.presencas:
          ...
          if st.button(f"Remover {c_nome}", key=f"rem_l_{c_nome}"):
            st.session_state.presencas = [
                item
                for item in st.session_state.presencas
                if obter_nome_p(item) != c_nome
            ]
            salvar_dados(PRESENCAS_FILE, st.session_state.presencas)
            st.rerun()
      else:
        ...
        if st.session_state.perfil_logado == "Jogadora":
          ...
          dados_j = next(
              (j for j in st.session_state.jogadoras if j["nome"] == j_nome),
              None,
          )
          ...
          if pos_conf:
            st.success(
                f"🎉 Você está na **Lista Principal** na posição **{pos_conf}**!"
            )
          elif pos_esp:
            st.warning(
                f"⏳ Você está na **Fila de Espera** na position **{pos_esp}º**."
            )
          else:
            st.info("ℹ️ Você não está confirmada.")

          with st.form("form_pres"):
            c_ok = st.form_submit_button(
                "👍 Confirmar Presença", use_container_width=True
            )
            c_canc = st.form_submit_button(
                "❌ Cancelar Presença", use_container_width=True
            )
    ...
    if sorteio_salvo and "times" in sorteio_salvo:
      ...
      for nome_time, membros in sorteio_salvo["times"].items():
        st.markdown(
            f"<div class='card-team'><h3>⚽ {nome_time}</h3>",
            unsafe_allow_html=True,
        )
        for item in membros:
          st.markdown(f"• **{item}**")
    ...
    if st.button("Gerar Sorteio Paralelo Agora", use_container_width=True):
      ...
      if len(confirmadas_nomes) >= 2:
        ...
        res_paralelo = {
            "Time A": confirmadas_nomes[::2],
            "Time B": confirmadas_nomes[1::2],
        }
        st.success("Sorteio Paralelo Gerado com Sucesso!")
        for nome_t, membros_t in res_paralelo.items():
          st.markdown(
              f"<div class='card-team'><b>{nome_t}:</b>"
              f" {', '.join(membros_t)}</div>",
              unsafe_allow_html=True,
          )
      else:
    ...
    for j in st.session_state.jogadoras:
      if j.get("status") == "Ativo":
        ...
        with col_e1:
          st.markdown(
              f"<div class='card-team'><b>⚽ {j['nome']}</b><br><small>Tipo:"
              f" `{j.get('tipo', 'Avulso')}` | Quitado:"
              f" `{j.get('quitado', 'Não')}` | Nasc:"
              f" {j.get('nascimento')}</small></div>",
              unsafe_allow_html=True,
          )
        with col_e2:
          if st.session_state.perfil_logado in ["Admin", "Dev"]:
            if st.button("🗑️ Excluir", key=f"exc_elenco_{j['nome']}"):
              st.session_state.jogadoras = [
                  atleta
                  for atleta in st.session_state.jogadoras
                  if atleta["nome"] != j["nome"]
              ]
              salvar_dados(DATA_FILE, st.session_state.jogadoras)
              st.session_state.presencas = [
                  p
                  for p in st.session_state.presencas
                  if obter_nome_p(p) != j["nome"]
              ]
              salvar_dados(PRESENCAS_FILE, st.session_state.presencas)
              st.success(f"Atleta {j['nome']} removida do sistema.")
              st.rerun()
    ...
    if st.session_state.perfil_logado == "Jogadora":
      ...
      with st.form("form_comprovante_envio", clear_on_submit=True):
        arquivo_submetido = st.file_uploader(
            "Selecione a imagem do comprovante", type=["png", "jpg", "jpeg"]
        )
        if st.form_submit_button("Enviar Comprovante"):
          if arquivo_submetido:
            caminho_arquivo = os.path.join(
                UPLOAD_DIR,
                f"{st.session_state.usuario_logado}_{int(datetime.now().timestamp())}.png",
            )
            ...
            st.success(
                "Comprovante enviado com sucesso para validação do"
                " Administrador!"
            )
          else:
    ...
    if st.session_state.perfil_logado in ["Admin", "Dev"]:
      ...
      for idx, comp in enumerate(comprovantes):
        if not comp.get("conferido", False):
          st.markdown(
              f"<div class='card-team'><b>Atleta:</b> {comp['nome']} |"
              f" <b>Data:</b> {comp['data']}</div>",
              unsafe_allow_html=True,
          )
          if os.path.exists(comp["arquivo"]):
            st.image(comp["arquivo"], width=300)
          if st.button(
              f"Validar Pagamento de {comp['nome']}", key=f"val_comp_{idx}"
          ):
            comp["conferido"] = True
            for j in st.session_state.jogadoras:
              if j["nome"] == comp["nome"]:
            ...
            st.success(
                "Pagamento validado e adicionado como receita no fluxo de"
                " caixa!"
            )
            st.rerun()
    ...
    with st.form("form_lanca_caixa", clear_on_submit=True):
      ...
      c_mes = st.text_input(
          "Mês / Ano (Ex: Janeiro/2026)", value=hoje_dt.strftime("%B/%Y")
      )
    ...
    if not registros_caixa:
      ...
      total_geral_rec = sum(
          item["valor"] for item in registros_caixa if item["tipo"] == "Receita"
      )
      total_geral_desp = sum(
          item["valor"] for item in registros_caixa if item["tipo"] == "Despesa"
      )
    ...
    tab_ger1, tab_ger2, tab_ger3, tab_ger4 = st.tabs([
        ...
        "🔒 Gestão de Contas (Dev)",
    ])

    with tab_ger1:
      st.write("### Aprovação de Novas Atletas")
      pendentes = [
          j for j in st.session_state.jogadoras if j.get("status") == "Pendente"
      ]
      if not pendentes:
      ...
      for idx, j in enumerate(pendentes):
        ...
        with col_p1:
          st.write(
              f"**{j['nome']}** (`{j.get('tipo', 'Avulso')}`) - Nasc:"
              f" {j.get('nascimento')}"
          )
        with col_p2:
          if st.button("✅ Aprovar", key=f"aprov_{idx}"):
            ...
            salvar_dados(DATA_FILE, st.session_state.jogadoras)
            st.success(
                f"✔️ A atleta {j['nome']} foi aprovada e ativada com sucesso!"
            )
            st.rerun()
        with col_p3:
          if st.button("❌ Recusar", key=f"rec_{idx}"):
            ...
            salvar_dados(DATA_FILE, st.session_state.jogadoras)
            st.warning(
                f"⚠️ O cadastro de {j['nome']} foi recusado/removido."
            )
            st.rerun()

    with tab_ger2:
      st.write("### ➕ Cadastro Direto de Atleta (Feito pelo Admin)")
      with st.form("form_cad_direto_admin", clear_on_submit=True):
        adm_nome = st.text_input("Nome Completo da Atleta *")
        adm_nasc = st.text_input("Nascimento (DD/MM)", placeholder="Ex: 15/05")
        adm_tipo = st.selectbox("Tipo de Atleta", ["Avulso", "Mensalista"])
        adm_login = st.text_input("Login de Acesso *")
        adm_senha = st.text_input("Senha Inicial *", type="password")

        if st.form_submit_button("CADASTRAR ATLETA DIRETAMENTE"):
          if adm_nome and adm_login and adm_senha:
            if any(
                j.get("login") == adm_login.strip()
                for j in st.session_state.jogadoras
            ):
              st.error("Este login já está em uso por outra jogadora!")
            else:
              st.session_state.jogadoras.append({
                  "nome": adm_nome.strip(),
                  "nascimento": adm_nasc.strip(),
                  "login": adm_login.strip(),
                  "senha": adm_senha.strip(),
                  "tipo": adm_tipo,
                  "status": "Ativo",
                  "quitado": "Não",
              })
              salvar_dados(DATA_FILE, st.session_state.jogadoras)
              st.success(
                  f"Atleta {adm_nome.strip()} cadastrada e ativada com sucesso!"
              )
              st.rerun()
          else:
            st.error("Preencha todos os campos obrigatórios (*).")
    ...
    with tab_ger3:
      with st.form("form_cfg_geral_painel"):
        limite_v = st.number_input(
            "Limite de Vagas",
            value=int(st.session_state.avisos.get("limite_vagas", 15)),
        )
        pix_val = st.text_input(
            "Chave Pix", value=st.session_state.avisos.get("pix", "")
        )
    ...
    with tab_ger4:
      if st.session_state.perfil_logado == "Dev":
        st.write("### Gerenciamento de Credenciais de Administradores")
        for idx, adm in enumerate(st.session_state.administradores):
          st.markdown(
              f"<div class='card-team'><b>Admin:</b> {adm['nome']} |"
              f" <b>Login:</b> <code>{adm['login']}</code></div>",
              unsafe_allow_html=True,
          )
